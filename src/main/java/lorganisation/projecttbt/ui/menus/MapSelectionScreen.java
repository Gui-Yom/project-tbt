...
public class MapSelectionScreen extends Screen {
    ...
    public MapSelectionScreen(Game game) {
        ...
        super();
        addComponent(new Text(new Coords(0, 2), new StyledString("Available maps"), Utils.Align.CENTER));

        int i = 0;
        for (Map.Entry<String, String> e : AssetsManager.gameMaps().entrySet()) { //max 10 maps pr l'instant
            addComponent(new Text(new Coords(0, 5 + i), new StyledString("[" + i + "] " + e.getKey()), Utils.Align.CENTER));
